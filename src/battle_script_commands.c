...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    case ABILITY_FOREWARN:
        if (defAbility == ABILITY_FOREWARN && (gBattleMoves[move].target == MOVE_TARGET_BOTH || gBattleMoves[move].target == MOVE_TARGET_FOES_AND_ALLY))
            calc = (calc * 90) / 100;
        break;
    }
...
void SetMoveEffect(bool32 primary, u32 certain)
{
    ...
    if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT) // status change
    {
        ...
        case STATUS1_BURN:
            ...
            if ((battlerAbility == ABILITY_WATER_VEIL || battlerAbility == ABILITY_WATER_BUBBLE || battlerAbility == ABILITY_MUDDY)
              && (primary == TRUE || certain == MOVE_EFFECT_CERTAIN))
            {
                gLastUsedAbility = battlerAbility;
                RecordAbilityBattle(gEffectBattler, battlerAbility);
...
static void Cmd_switchineffects(void)
{
        ...
        && IsBattlerGrounded(battler))
    {
        ...
        && IsBattlerAffectedByHazards(battler, FALSE)
        && GetBattlerAbility(battler) != ABILITY_MAGIC_GUARD
        && GetBattlerAbility(battler) != ABILITY_FLOATY)
    {
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_TRY_ACTIVATE_FELL_STINGER:
    {
        ...
        if (gBattleMoves[gCurrentMove].effect == EFFECT_SOULBIND
            && HasAttackerFaintedTarget()
            && !NoAliveMonsForEitherParty()
            && CompareStat(gBattlerAttacker, STAT_SPATK, MAX_STAT_STAGE, CMP_LESS_THAN))
        {
            SET_STATCHANGER(STAT_SPATK, (B_SOULBIND_STAT_RAISE >= GEN_7 ? 3 : 2), FALSE);
            PREPARE_STAT_BUFFER(gBattleTextBuff1, STAT_SPATK);
            BattleScriptPush(cmd->nextInstr);
            gBattlescriptCurrInstr = BattleScript_FellStingerRaisesStat;
            return;
    ...
    if (statValue <= -1) // Stat decrease.
    {
                 ...
                 && notProtectAffected != TRUE && JumpIfMoveAffectedByProtect(gCurrentMove))
        {
            ...
            return STAT_CHANGE_DIDNT_WORK;
              ...
              || CanAbilityPreventStatLoss(battlerAbility, GetBattlerAbility(gBattlerAttacker) == ABILITY_INTIMIDATE || GetBattlerAbility(gBattlerAttacker) == ABILITY_DEMORALIZE))
